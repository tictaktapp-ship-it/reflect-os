...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { meeting_notes, mode, categories } = body;

    if (!meeting_notes || typeof meeting_notes !== "string") {
      return json({ error: "meeting_notes (string) required" }, 400);
    }
    ...
    const systemPrompt =
      "You are an expert at identifying and extracting key decisions from meeting notes. " +
      "Return only valid JSON with no markdown or explanation.";

    const userContent = multiple
      ? `Extract all decisions made or discussed in the following meeting notes. ` +
        `For each decision, provide a structured summary.\n\n` +
        ...
        `  ]\n` +
        `}`
      : `Extract the single most significant decision from the following meeting notes.\n\n` +
        (categoryList ? `Available categories: ${categoryList}\n\n` : "") +
        `Meeting notes:\n${meeting_notes}\n\n` +
        `Return ONLY valid JSON with this structure:\n` +
        `{\n` +
    ...
    if (!Array.isArray(parsed.decisions) || parsed.decisions.length === 0) {
      return json({ error: "No decisions found in the notes" }, 422);
    }
